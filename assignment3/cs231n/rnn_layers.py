...
def rnn_step_backward(dnext_h, cache):
    ...
    x, prev_h, Wx, Wh, next_h = cache.get("x"), cache.get("prev_h"), cache.get("Wx"), cache.get("Wh"), cache.get("next_h")
...
def rnn_backward(dh, cache):
    ...
    for t in reversed(range(len(cache))):
        ...
        cur_dx, cur_dprev_h, cur_dWx, cur_dWh, cur_db = rnn_step_backward(dh_current, cache[t])
        dx[:, t, :] = dx[:, t, :] + cur_dx
        dWx = dWx + cur_dWx
        dWh = dWh + cur_dWh
        db = db + cur_db
...
def lstm_backward(dh, cache):
    ...
    for t in reversed(range(len(cache))):
        ...
        dprev_x, dprev_h, dnext_c, dprev_Wx, dprev_Wh, dprev_b = lstm_step_backward(dnext_h, dnext_c, cache[t])
        dx[:, t, :] += dprev_x
        dWx += dprev_Wx
        dWh += dprev_Wh
        db += dprev_b
...
def temporal_softmax_loss(x, y, mask, verbose=False):
    ...
    # print("log shape is {}".format(probs[np.arange(N * T), y_flat].shape))
    # print("mask_flat shape is {}".format(mask_flat.shape))
