...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        # Step 1
        h0 = features.dot(W_proj) + b_proj  # h0: N x H
        # print('h0 shape is {}'.format(h0.shape))

        # Step 2
        x, x_embedding = word_embedding_forward(captions_in, W_embed)   # x: N x T x W
        # print('x shape is {}'.format(x.shape))

        # Step 3
        if self.cell_type == 'rnn':
            h, h_cache = rnn_forward(x, h0, Wx, Wh, b)   # h: N x T x H
        ...
        # Step 4
        scores, scores_cache = temporal_affine_forward(h, W_vocab, b_vocab) # scores: N x T x V
        # print(scores.shape)
        ...
        dh, dW_vocab, db_vocab = temporal_affine_backward(dloss, scores_cache)
        ...
        dW_embed = word_embedding_backward(dx, x_embedding)
